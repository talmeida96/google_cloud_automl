...
while True:
    ...
    # Atualiza a posição da ROI a cada x segundos (opcional)
    if end_time > 3:
    ...
    # Salva a imagem recortada no diretório de saída
    new_img = f"{output_dir}roi_2_{img_number}.jpg"
    cv2.imwrite(new_img, roi)

    # Exibe o frame na tela
    cv2.imshow('Gerador de Datasets com ROI (1)', frame)

    # Adiciona intervalo de x segundos entre as capturas
    time.sleep(5)
